...
文件範圍外的問題，例如 `想睡覺了怎麼辦？` 或 `巧克力蛋糕怎麼做？`，會在 Retrieval relevance gate（檢索相關性閘門）被拒絕，不呼叫 LLM、不回傳來源，並回答本地知識庫沒有足夠相關資訊。
...
- Retrieval 的 top-1 必須同時達到最低分數 `1.5` 與 query term coverage（問題詞覆蓋率）`0.30`，否則視為資料不足。
- Server 只在目前問題具有追問特徵且有有效文件詞命中時，才使用最近一輪問題補強 Retrieval，避免舊對話把新主題錯誤帶進知識庫。
- LLM 收到有限的對話歷史與 Retrieval 找出的 top 3 片段，prompt 明確禁止使用模型背景知識補充答案。
